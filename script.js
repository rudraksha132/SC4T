...
function initSite() {
		...
		function animate() {
				...
				const entryTl = gsap.timeline({
						scrollTrigger: { trigger: ".sub", start: "top top", end: "+=200%", pin: true, scrub: 1 }
				});
				...
				titleHeadings.forEach((heading) => {
						ScrollTrigger.create({
								...
								start: "top center",
								onEnter: (self) => {
										const split = splitTextMap.get(self.trigger);
										if (split) {
												gsap.fromTo(split.words, 
														{ y: "100%", opacity: 0, filter: "blur(4px)" },
														{ y: "0%", opacity: 1, filter: "blur(0px)", ease: "power3.out", duration: 1, stagger: { each: 0.075, from: "left" } }
												);
										}
								},
								onLeaveBack: (self) => {
										const split = splitTextMap.get(self.trigger);
										if (split) {
												gsap.to(split.words, { y: "100%", opacity: 0, filter: "blur(4px)", ease: "power3.inOut", duration: 0.5, stagger: { each: 0.075, from: "right" } });
										}
								}
						});
				});
